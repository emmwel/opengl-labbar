...
void DeformCylinder()
{
  ...
  //float w[kMaxBones];
  int row, corner;
	// mat4 boneTranslations[kMaxBones-1];
	mat4 Mmb[kMaxBones-1];
	mat4 Mbm[kMaxBones-1];

	for (int bone = 1; bone < kMaxBones; bone++) {
		vec3 diff = VectorSub(g_bones[bone].pos, g_bones[bone-1].pos);
		mat4 trans = T(diff.x, diff.y, diff.z);
		mat4 Mbonei = Mult(trans, g_bones[bone].rot);
		if (bone == 1) {
			Mbm[bone-1] = Mult(Mbonei, g_bonesRes[bone].rot);
			Mmb[bone-1] = InvertMat4(Mbonei);
		}
		else {
			Mbm[bone-1] = Mult(Mbm[bone-2], Mult(Mbonei, g_bonesRes[bone].rot));
			Mmb[bone-1] = Mult(InvertMat4(Mbonei), Mmb[bone-2]);
		}
    ...
    for (corner = 0; corner < kMaxCorners; corner++)
    {
      ...
      // g_vertsRes
			...
			g_vertsRes[row][corner] = ScalarMult(
				MultVec3(g_bonesRes[0].rot, g_vertsOrg[row][corner]),
				g_boneWeights[row][corner][0]
			);

			for(int bone = 1; bone <  kMaxBones; bone++) {
				vec3 res = ScalarMult(
					MultVec3(
						Mult(Mbm[bone-1], Mmb[bone-1]), // Mi
						g_vertsOrg[row][corner] // v
					),
					g_boneWeights[row][corner][bone] // wi
				);
				g_vertsRes[row][corner] = VectorAdd(g_vertsRes[row][corner], res);
			}
