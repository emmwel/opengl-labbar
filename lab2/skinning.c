...
void DeformCylinder()
{
	...
	mat4 neg0 = T(-g_bones[0].pos.x, -g_bones[0].pos.y, -g_bones[0].pos.z);
	mat4 pos0 = T(g_bones[0].pos.x, g_bones[0].pos.y, g_bones[0].pos.z);
	mat4 neg1 = T(-g_bones[1].pos.x, -g_bones[1].pos.y, -g_bones[1].pos.z);
	mat4 pos1 = T(g_bones[1].pos.x, g_bones[1].pos.y, g_bones[1].pos.z);

	mat4 Mbone0 = Mult(pos0, Mult(g_bones[0].rot, neg0));
	mat4 Mbone1 = Mult(pos1, Mult(g_bones[1].rot, neg1));

	// f�r samtliga vertexar
	for (row = 0; row < kMaxRow; row++)
	{
		int bone_index = weight[row];
		...
		for (corner = 0; corner < kMaxCorners; corner++)
		{
			...
			// if (bone_index == 0) {
			// 	g_vertsRes[row][corner] = MultVec3(Mbone0, g_vertsRes[row][corner]);
			// }
			// else {
			// 	g_vertsRes[row][corner] = MultVec3(Mbone1, g_vertsRes[row][corner]);
			// }
			...
			// g_vertsOrg inneh�ller ursprunglig vertexdata.
			// g_vertsRes inneh�ller den vertexdata som skickas till OpenGL.
			// vec3 term1 = ScalarMult(
			// 	MultVec3(Mbone0, g_vertsRes[row][corner]),
			// 	g_boneWeights[row][corner].x
			// );
			//
			// vec3 term2 = ScalarMult(
			// 	MultVec3(Mbone1, g_vertsRes[row][corner]),
			// 	g_boneWeights[row][corner].y
			// );
			//
			// g_vertsRes[row][corner] = VectorAdd(term1, term2);
...
void setBoneRotation(void)
{
	...
	// till vertexshadern
  glUniformMatrix4fv(glGetUniformLocation(g_shader, "b1_rot"), 1, GL_TRUE, g_bones[1].rot.m);
}
...
void setBoneLocation(void)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(g_shader, "b1_neg1"), 1, GL_TRUE, neg1.m);
	glUniformMatrix4fv(glGetUniformLocation(g_shader, "b1_pos1"), 1, GL_TRUE, pos1.m);
}
